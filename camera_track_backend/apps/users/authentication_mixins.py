...
class Permissions(object):
    ...
    def dispatch(self, request, *args, **kwargs):
        user = self.get_user(request)
        print("Aqui Usuario:", user.get_all_permissions())
        print("Aqui permisos:", user.has_perms(self.get_perms()))
        # perms = [perm for perm in self.get_perms()]
        # print(perms)
        if user.has_perms(self.get_perms()):
            return super().dispatch(request, *args, **kwargs)
        response = Response(
            {'error': 'Gaaaaa.'},
            status=status.HTTP_400_BAD_REQUEST
